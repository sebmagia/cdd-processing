...
for cdd_name in cdd_names:
    ...
    ddepth, dsigmaln = all_gm_suite.sigma_ln(dmax=120,dy=0.1)
    ...
    for n,suit in enumerate(suites):
        ...
        print(nlays[n])
        if nlays[n] == 7:
                ax.plot(gm.vs2, gm.depth,label=label, color =plt.cm.jet(gm.misfit/(max_misfit) ))
        if nlays[n] == 6:
                ax.plot(gm.vs2, gm.depth,label=label, color  =plt.cm.jet(gm.misfit/(max_misfit) ))
        if nlays[n] == 5:
                ax.plot(gm.vs2, gm.depth, label=label, color =plt.cm.jet(gm.misfit/(max_misfit) ))
    ...
    #colors=[]
    err=[ np.sum((x - c_obs)**2) for x in  c_cals]
    max_err = max(err)
    min_err = min(err)
    ...
    for n,co in enumerate(c_cals):
        label = 'N' + str(nlays[n]) + '_' + 'S' + str(seeds[n])
        if nlays[n] == 7:
            ax.plot(freq, co, label=label, color= plt.cm.jet(err[n] / (max_err)))
        if nlays[n] == 6:
            ax.plot(freq, co, label=label, color= plt.cm.jet(err[n] / (max_err)))
        if nlays[n] == 5:
            ax.plot(freq, co, label=label, color= plt.cm.jet(err[n] / (max_err)))
        ...
        if nlays[n] == 3:
            ax.plot(freq, co, label=label, color= plt.cm.jet(err[n] / (max_err)))
    sm = plt.cm.ScalarMappable(cmap=plt.cm.jet, norm=plt.Normalize(vmin=min_err, vmax=max_err))
    fig.colorbar(sm, ax=ax)
    ...
    for n,co in enumerate(c_cals):
        label = 'N' + str(nlays[n]) + '_' + 'S' + str(seeds[n])
        if nlays[n] == 7:
            ax.plot(freq, np.abs(co - c_obs), label=label, color=plt.cm.jet(err[n] / (max_err)))
        if nlays[n] == 6:
            ax.plot(freq, np.abs(co - c_obs), label=label, color=plt.cm.jet(err[n] / (max_err)))
        if nlays[n] == 5:
            ax.plot(freq, np.abs(co - c_obs), label=label, color=plt.cm.jet(err[n] / (max_err)))
