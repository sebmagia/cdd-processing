...
if create_clones:
    ...
    for clones in lineas:
        clones = [x for x in clones if estados[x] != 12]
        clones_new.append(clones)
    from itertools import chain

    clones_new_flat = list(chain(*clones_new))
    all_idx = list(range(len(coords)))
    not_clones = [[x] for x in all_idx if x not in clones_new_flat]
    todas = clones_new + not_clones
    #todas_sorted = sorted(todas, key=lambda x: x[0])
    todas_set = sorted([list(t) for t in set(tuple(i) for i in todas)])
    todas_set_usable = [[x for x in sub_list if estado[x] <= 2.5] for sub_list in todas_set]
    todas_set_usable=[x for x in todas_set_usable if len(x) > 0]
    todas_sorted = sorted(todas_set_usable, key=lambda x: x[0])

    for x in todas_sorted:
        midx = [np.mean(freq_stack[x, i]) for i in range(freq_stack.shape[1])]
        midy = [np.mean(vel_stack[x, i]) for i in range(freq_stack.shape[1])]
        midstd = [np.max(vel_std_stack[x, i]) for i in range(freq_stack.shape[1])]
        cx1 = np.mean(coords_sorted[x, 0])
        cx2 = np.mean(coords_sorted[x, 2])
        cy1 = np.mean(coords_sorted[x, 1])
        cy2 = np.mean(coords_sorted[x, 3])
    ...
    for f, v in zip(freqs_merged, vels_merged):
        plt.plot(f, v, 'b')
        plt.text(f[0], v[0], str(todas_sorted[n][0]))
        n += 1
        print(n)


if create_files:
        #coords_merged_sp=np.split(coords_merged,2)
        ...
        for i in range(len(freqs_merged)):
            ...
            fig1.suptitle('Interstation Distance ' + str(np.round(dist,2)) + ' Curvas: ' + "-".join( str(x) for x in todas_sorted[i]) )
