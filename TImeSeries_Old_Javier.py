...
step_1=True
step_2=False
step_3=False ## only fixing data is required
step_4=False
...
if step_1:
    ...
    # data5=data[:-1-fs*3,4]
    # #
    data1=np.genfromtxt(path+'M7_001part1/EqualizedFile.dat',skip_header=34,encoding='ISO-8859-1',usecols=2)
    data2=np.genfromtxt(path+'M7_002part1/EqualizedFile.dat',skip_header=34,encoding='ISO-8859-1',usecols=2)
    data3=np.genfromtxt(path+'M7_003part1/EqualizedFile.dat',skip_header=34,encoding='ISO-8859-1',usecols=2)
    data4=np.genfromtxt(path+'M7_004part1/EqualizedFile.dat',skip_header=34,encoding='ISO-8859-1',usecols=2)
    data5=np.genfromtxt(path+'M7_005part1/EqualizedFile.dat',skip_header=34,encoding='ISO-8859-1',usecols=2)
    data6=np.genfromtxt(path+'Seba4_001part1/EqualizedFile.dat',skip_header=42,encoding='ISO-8859-1',usecols=2)

    #data=np.vstack((data1, data2, data3, data4,data5,data6)).T

    #f,C=fl.raw_coherence(data,fs)

    starttime1="2022/03/15 17:27:20"
    endtime1="2022/03/15 18:11:20"

    starttime2 = "2022/03/15 17:26:47"
    endtime2 = "2022/03/15 18:10:47"

    starttime3 = "2022/03/15 17:25:04"
    endtime3 = "2022/03/15 18:09:04"

    starttime4 = "2022/03/15 17:29:16"
    endtime4 = "2022/03/15 18:13:16"

    starttime5 = "2022/03/15 17:26:57"
    endtime5 = "2022/03/15 18:10:57"

    starttime6 = "2022/03/15 17:26:45"
    endtime6 = "2022/03/15 18:10:45"
    ...
    trace2 = obspy.Trace(data=data2, header={'sampling_rate': fs, 'network': nw, 'station': 'E2',
                                             'starttime':obspy.UTCDateTime(starttime2),'endtime':obspy.UTCDateTime(endtime2)})
    trace3 = obspy.Trace(data=data3, header={'sampling_rate': fs, 'network': nw, 'station': 'E3',
                                             'starttime':obspy.UTCDateTime(starttime3),'endtime':obspy.UTCDateTime(endtime3)})
    trace4 = obspy.Trace(data=data4, header={'sampling_rate': fs, 'network': nw, 'station': 'E4',
                                             'starttime':obspy.UTCDateTime(starttime4),'endtime':obspy.UTCDateTime(endtime4)})
    trace5 = obspy.Trace(data=data5, header={'sampling_rate': fs, 'network': nw, 'station': 'E5',
                                             'starttime':obspy.UTCDateTime(starttime5),'endtime':obspy.UTCDateTime(endtime5)})

    trace6 = obspy.Trace(data=data6, header={'sampling_rate': fs, 'network': nw, 'station': 'E6',
                                             'starttime': obspy.UTCDateTime(starttime6),
                                             'endtime': obspy.UTCDateTime(endtime6)})

    stream=obspy.Stream(traces=[trace1,trace2,trace3,trace4,trace6])


    starts=[x.stats.starttime for x in  stream]
    ends=[x.stats.endtime for x in stream]

    start_coh=max(starts)
    end_coh=min(ends)
    stream2=stream.copy()
    stream2=stream2.slice(start_coh,end_coh)
    stream2.write(path+MED+'.mseed',c)
    ...
    #
    #data_new=np.vstack((data1,data2,data3,data4,data5,data6)).T
    #
    # if check_coh:
...
if step_2:
    ...
    fig, ax = plt.subplots(len(combs) // 2+1, 2, figsize=(10, 8))
    m = 70
    for n, x in enumerate(combs):
        ...
        #R=correlate(stream[int(x[0][1])-1].data,stream[int(x[1][1])-1].data)
        #lags = correlation_lags(len(stream[int(x[0][1])-1].data), len(stream[int(x[0][1])-1].data))
        f, coh, cohp = fl.cross_coherence(stream, fs, nsegs, x, norm='1bit')
        try:
        ...
        if n < len(combs) // 2:
            ...
            ax[n - 7, 1].plot(f[:m], coh[:m], linewidth='2')
            ax[n - 7, 1].text(30, 0.25, 'Intersta_Dist:' + str(np.round(dist, 2)) + '[m]', fontsize='8')
...
if step_3:
    ...
    stream2=stream.copy()
    coords = np.loadtxt(path + 'coords_' + MED + '.txt')
    stream[0].data=stream[0].data[fs*3:-1]
    stream[1].data=stream[1].data[:-1-fs*3]
    stream[2].data=stream[2].data[:-1-fs*3]
    stream[3].data=stream[3].data[:-1-fs*3]
    stream[4].data=stream[4].data[:-1-fs*3]
    #stream[5].data=stream[5].data[:-1-fs*3]
    ND = [x.stats.station for x in stream]
    combs = list(itertools.combinations(ND, 2))
    combs_coords = list(itertools.combinations(coords, 2))
    fig, ax = plt.subplots(len(combs) // 2 + 1, 2, figsize=(10, 8))
